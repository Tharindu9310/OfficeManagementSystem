...
- `.claude/skills/design/technical-design/SKILL.md`
...
### Multi-Phase Rule (when the clarified requirement names a prior phase)
If `clarified-requirement.md` identifies this as phase 2+ of an existing initiative (base feature name + prior phase's plugin `SystemName`), default the Placement Decision to **"Extension of existing plugin"** targeting that same `SystemName`, not "New plugin" — a new phase of the same feature should almost always extend the plugin phase 1 already created. Only deviate from this (and scaffold a genuinely separate plugin) if there's a concrete architectural reason to, and state that reason explicitly, since diverging from an established phase's plugin identity is a decision worth justifying, not a default.
...
## Expected Outputs
**File Creation Rule (MANDATORY):** Use the Write/Edit tool to physically write the design document to `docs/`, relative to the workspace root. Do not return it as chat text only. Confirm the write actually landed in the workspace (diff Claude Code shows before applying an edit), not merely generated.

Create the following files using kebab-case feature name:
1. `docs/{feature-name}/design/technical-design.md` — including Steps 1–5 above
